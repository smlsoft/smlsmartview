...
import type {
  PurchaseDebtLine,
  PurchaseDocumentDetailData,
  PurchaseDocumentHeader,
  PurchaseItemLine
} from "@/lib/queries/purchase";

type PurchaseDocumentDetailProps = {
  data: PurchaseDocumentDetailData;
  backHref: string;
};
...
function DocumentOverview({ header }: { header: PurchaseDocumentHeader }) {
  return (
    <Card className="p-6">
...
function TotalsPanel({ header }: { header: PurchaseDocumentHeader }) {
  return (
    <Card className="p-6">
      <CardHeader className="space-y-0 p-0">
...
function StatusPanel({ header }: { header: PurchaseDocumentHeader }) {
  const rows = [
...
function warehouseLabel(line: PurchaseItemLine) {
  const from = [line.wh_code, line.shelf_code].filter(Boolean).join("/");
  return from || "-";
}

function ItemLinesTable({ lines }: { lines: PurchaseItemLine[] }) {
  return (
...
function DebtLinesTable({ lines }: { lines: PurchaseDebtLine[] }) {
  return (
    <Card className="overflow-hidden p-0">
      <CardHeader className="flex flex-row items-start justify-between gap-4 p-6 pb-4">
        <div>
          <CardTitle>รายการตัดหนี้/อ้างอิง</CardTitle>
          <p className="mt-2 text-sm text-text-secondary">
            {formatNumber(lines.length)} lines
          </p>
        </div>
      ...
      <CardContent className="p-0">
        <div className="overflow-x-auto premium-scrollbar">
          <table className="w-full min-w-[920px] border-t border-border text-sm">
            <thead className="bg-surface-muted text-left">
              <tr className="label-caps text-text-tertiary">
                <th className="px-5 py-3 font-semibold">#</th>
                ...
                <th className="px-5 py-3 text-right font-semibold">จ่าย/คืน</th>
              ...
              {lines.length === 0 ? (
                <tr>
                  <td colSpan={8} className="px-5 py-10 text-center text-text-secondary">
                    ไม่พบรายการตัดหนี้หรือรายการอ้างอิงในเอกสารนี้
                  </td>
                </tr>
              ) : (
                lines.map((line) => (
                  <tr key={`${line.line_number}-${line.billing_no}`}>
                    <td className="px-5 py-4 align-top font-mono text-text-tertiary">
                      {line.line_number || "-"}
                    </td>
                    <td className="px-5 py-4 align-top">
                      <p className="font-mono text-text-primary">{line.billing_no || "-"}</p>
                      <p className="mt-1 text-xs text-text-tertiary">
                        {formatDate(line.billing_date)}
                      </p>
                    </td>
                    <td className="px-5 py-4 align-top text-text-secondary">
