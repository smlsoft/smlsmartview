...
import type {
  ArDebtLine,
  ArDocumentDetailData,
  ArDocumentHeader,
  ArItemLine
} from "@/lib/queries/ar";

type ArDocumentDetailProps = {
  data: ArDocumentDetailData;
  backHref: string;
};
...
function DocumentOverview({ header }: { header: ArDocumentHeader }) {
  return (
    <Card className="p-6">
      ...
      <CardContent className="grid gap-3 p-0 pt-6 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <InfoItem
          icon={UserRound}
          label="ลูกหนี้"
          value={valueOrDash(header.customer_name || header.customer_code)}
        />
...
function TotalsPanel({ header }: { header: ArDocumentHeader }) {
  return (
    <Card className="p-6">
      <CardHeader className="space-y-0 p-0">
...
function PaymentPanel({ header }: { header: ArDocumentHeader }) {
  return (
    <Card className="p-6">
      <CardHeader className="space-y-0 p-0">
...
function StatusPanel({ header }: { header: ArDocumentHeader }) {
  const rows = [
...
function warehouseLabel(line: ArItemLine) {
  const from = [line.wh_code, line.shelf_code].filter(Boolean).join("/");
  return from || "-";
}

function ItemLinesTable({ lines }: { lines: ArItemLine[] }) {
  return (
...
function DebtLinesTable({ lines }: { lines: ArDebtLine[] }) {
  return (
    <Card className="overflow-hidden p-0">
      <CardHeader className="flex flex-row items-start justify-between gap-4 p-6 pb-4">
        <div>
...
export function ArDocumentDetail({ data, backHref }: ArDocumentDetailProps) {
  return (
