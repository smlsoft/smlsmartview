import { ArrowLeft, Building2, HandCoins, Mail, Phone, ReceiptText } from "lucide-react";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { StatusChip } from "@/components/ui/status-chip";
import type { ApSupplierDetailData } from "@/lib/queries/ap";

type ApSupplierDetailProps = {
  data: ApSupplierDetailData;
  backHref: string;
};
...
export function ApSupplierDetail({ data, backHref }: ApSupplierDetailProps) {
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <AmountCard title="ยอดหนี้" value={supplier.debt_balance} />
        <AmountCard title="ยอดเช็ครอขึ้นเงิน" value={supplier.chq_balance} />
        <AmountCard title="ยอดรับวางบิลรอจ่าย" value={supplier.bill_balance} />
      </section>

      <section className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="grid gap-4 sm:grid-cols-2">
          <InfoCard title="ชื่อ 1" value={supplier.name_1} />
          <InfoCard title="ชื่อ 2" value={supplier.name_2} />
          <InfoCard title="ที่อยู่" value={supplier.address} />
          <InfoCard title="ประเภทเจ้าหนี้" value={supplier.ap_type || String(supplier.ap_status || "")} />
          <InfoCard
            title="สร้างข้อมูล"
            value={formatDateTime(supplier.create_datetime)}
            sub="create_datetime"
          />
          <InfoCard
            title="แก้ไขล่าสุด"
            value={formatDateTime(supplier.last_update_date_time)}
