...
function LoginInner({ defaultProviderCode, preferredDbCode }: LoginClientProps) {
  ...
  const [provider, setProvider] = useState(defaultProviderCode);
  ...
  async function submitCredentials(e: React.FormEvent) {
    ...
    try {
      ...
      const preferredBranch =
        data.branches.find((branch: Branch) => branch.db_code === preferredDbCode) ??
        data.branches[0];
